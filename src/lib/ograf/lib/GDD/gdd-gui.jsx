	...
	if (props.inTableRow || props.inTableCell) {
		...
		return (
			<div className={'gdd-property gdd-property-' + props.className}>
				...
				<div className="edit">{props.children}</div>
				{props.dataValidation && <div className="data-validation">{props.dataValidation}</div>}
			</div>
		)
	}
}
...
const propertyUnknown = (props, basicType) => {
	return (
		<EditProperty className="boolean" key={props.property} {...props}>
...
const propertyString = (props) => {
	const data = props.data || ''
	return (
		<EditProperty className="string" key={props.property} {...props}>
			<input
				type="text"
				value={data}
				onChange={(e) => {
...
const propertyNumber = (props) => {
	const data = props.data || ''
	return (
		<EditProperty className="number" key={props.property} {...props}>
			<input
				type="number"
				value={data}
				onChange={(e) => {
...
const propertyInteger = (props) => {
	const data = props.data || ''
	return (
		<EditProperty className="integer" key={props.property} {...props}>
			<input
				type="number"
				value={data}
				onChange={(e) => {
...
const propertyArray = (props) => {
	...
	return (
		<EditProperty className="array" key={props.property} {...props}>
			<table>
				<thead>
					<tr>
						{columns.map(([columnKey, column]) => {
							return (
								<th className="header" key={columnKey}>
									...
									{column.description && <div className="description">{column.description}</div>}
								</th>
							)
						})}
					</tr>
					...
					<tr>
						<td colSpan="99">
							<button
								className="add"
								onClick={() => {
									data.push(getDefaultDataFromSchema(props.schema.items) ?? null)
									props.setData(data)
								}}
							>
								+
...
const propertyObject = (props) => {
	...
	if (props.inTableRow) {
		...
		return (
			<EditProperty className="object" key={props.property} {...props}>
